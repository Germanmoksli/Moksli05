...
def add_room():
    ...
    if request.method == "POST":
        ...
        # Determine whether the kitchen is a studio.  Use a boolean rather
        # than an integer so that PostgreSQL accepts the value for a
        # boolean column.
        ...
        try:
            with conn:
                ...
                # Retrieve the newly created id.  When using PostgreSQL, the
                # RealDictCursor returns a dict (e.g. {'id': ...}); for SQLite
                # compatibility or older code paths, it may return a tuple.
                new_id_row = cur.fetchone()
                room_id = None
                if new_id_row:
                    # First try dict-like access (psycopg2 RealDictRow)
                    try:
                        room_id = new_id_row.get('id')  # type: ignore[attr-defined]
                    except Exception:
...
def edit_room(room_id: int):
    ...
    """
    conn = get_db_connection()
    # Ensure the rooms table contains additional columns so that select/update
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Roll back and show an error if any update fails
            try:
                conn.rollback()
            except Exception:
                pass
        ...
        return redirect(url_for("list_rooms"))
    # GET request: prepare display values for address
    address_parts = []
...
def public_listings():
    ...
    """
    conn = get_db_connection()
    # Ensure the rooms table has all additional columns before querying
    try:
        ensure_rooms_additional_columns(conn)
    except Exception:
        pass
