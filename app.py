...
def ensure_rooms_additional_columns(conn) -> None:
    ...
    the ``room_photos`` table exists.  This helper examines the existing
    schema and adds missing columns (e.g., num_rooms, floor, etc.) via
    ``ALTER TABLE``.  It also creates the ``room_photos`` table if it is
    absent.  Any errors encountered are ignored to avoid disrupting
    normal operation.
...
def ensure_room_photos_image_data_column(conn) -> None:
    ...
    """
    try:
        # Attempt to add the image_data column; if it exists, an exception
        # will be raised and ignored.  The syntax without IF NOT EXISTS is
        # used because not all SQL dialects support it uniformly.
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if new_room_id:
                ...
                # base64 strings can be stored.  Ignore errors if it
                # already exists.
                try:
                    ensure_room_photos_image_data_column(conn)
                except Exception:
                ...
                for fname, img_data in saved_photos:
                    try:
                        # Insert both the filename and the base64 image data.  If
                        # img_data is None, the column will receive NULL.
                        conn.execute(
...
def edit_room(room_id: int):
    ...
    if request.method == "POST":
        ...
        price_per_night = _to_float(request.form.get("price_per_night"))
        condition = request.form.get("condition") or None
        # Use boolean for kitchen_studio so that PostgreSQL boolean column accepts it
        kitchen_studio = True if request.form.get("kitchen_studio") == "yes" else False
        country = request.form.get("country") or None
...
def public_listings():
    ...
    if check_in and check_out:
        try:
            ...
            valid_dates = False
    # Ensure the room_photos table has an image_data column before selecting
    # photo content.  This call is safe to run multiple times and will be
    ...
    rooms = []
    for row in rows:
        # Convert row to a dictionary.  sqlite3.Row supports keys(); other
        # row-like objects (e.g. tuples from psycopg2) can be converted
        # manually.  If conversion fails, fallback to using attribute access.
        try:
            room_dict = dict(row)
        except Exception:
            room_dict = {}
            # If the row supports keys() (sqlite3.Row), iterate through keys
            if hasattr(row, 'keys'):
                for key in row.keys():
                    try:
                        room_dict[key] = row[key]
                ...
                # Fallback: assign numeric indices (unused).  Without column
                # names it is difficult to build a dict; skip in this case.
                pass
        # Determine the first photo's filename and image data from the row.
        file_name = None
        image_data = None
        # Attempt dict-style access first
        try:
            file_name = room_dict.get('photo_file_name')  # type: ignore[attr-defined]
            image_data = room_dict.get('photo_image_data')  # type: ignore[attr-defined]
        except Exception:
            # Fallback: try attribute or index access
            try:
                file_name = row['photo_file_name']  # type: ignore[index]
            except Exception:
                try:
                    # If row is a tuple and we know the position of the
                    # columns, we can't reliably retrieve them; skip.
                    pass
                except Exception:
                    pass
            try:
...
def view_room_public(room_id: int):
    """
    ...
    try:
        ...
        pass
    # Fetch the room record; convert to a dictionary for template rendering.
    room_row = conn.execute("SELECT * FROM rooms WHERE id = ?", (room_id,)).fetchone()
    if not room_row:
        conn.close()
        abort(404)
    # Ensure extended columns and the image_data column exist before selecting
    try:
        ensure_rooms_additional_columns(conn)
    except Exception:
        pass
    try:
        ...
        pass
    # Convert room_row to a dict for easier access in Jinja templates.
    try:
        room = dict(room_row)
    except Exception:
        room = {}
    ...
    if raw_amenities:
        ...
        # Parse the amenities string into a list of selected amenity codes.  Trim
        # whitespace and ignore empty values.  This list will be used to
        # filter the amenity categories defined below.  Separating this logic
        # here (in Python) avoids complex string manipulation in the Jinja
        # template and ensures consistent behaviour regardless of the data
        # type returned by the database.
        selected_amenities: list[str] = []
        try:
            for item in str(raw_amenities).split(','):
                cleaned = item.strip()
                if cleaned:
                    selected_amenities.append(cleaned)
        except Exception:
            selected_amenities = []

        # Define all available amenity categories and their options.  This
        # mirrors the structure used in the listing wizard (step 3) so that
        # values saved by the owner can be matched to human‑readable labels
        ...
        amenities_categories_full = [
            # Основное (основные опции)
            {
                ...
                'options': [
                    ...
                    {'value': 'microwave', 'label': 'Микроволновая печь', 'icon': 'bi-microwave'},
                    {'value': 'iron', 'label': 'Утюг', 'icon': 'bi-iron'},
                    {'value': 'dishes', 'label': 'Посуда', 'icon': 'bi-cup-straw'},
                    {'value': 'air_conditioner', 'label': 'Кондиционер', 'icon': 'bi-snow'},
            ...
            # Безопасность
            {
                'name': 'Безопасность',
                'options': [
                    {'value': 'intercom', 'label': 'Домофон', 'icon': 'bi-door-closed'},
                    {'value': 'video_intercom', 'label': 'Видеодомофон', 'icon': 'bi-camera-video'},
                    {'value': 'electronic_locks', 'label': 'Электронные замки', 'icon': 'bi-lock'},
                    {'value': 'parking', 'label': 'Паркинг', 'icon': 'bi-car-front'},
                    {'value': 'parking_nearby', 'label': 'Рядом есть парковка', 'icon': 'bi-car-front'},
                    {'value': 'smoke_detector', 'label': 'Датчик дыма', 'icon': 'bi-bell'},
                    {'value': 'gas_detector', 'label': 'Датчик газа', 'icon': 'bi-exclamation-octagon'},
            ...
            {
                'name': 'Кому подойдёт квартира',
                'options': [
                    {'value': 'non_smoking', 'label': 'Некурящим', 'icon': 'bi-slash-circle'},
                    {'value': 'for_parties', 'label': 'Для вечеринок', 'icon': 'bi-music-note-beamed'},
                    {'value': 'nightly_rental', 'label': 'Могу сдать на ночь', 'icon': 'bi-moon'},
                    {'value': 'with_children', 'label': 'Можно с детьми', 'icon': 'bi-people'},
                    {'value': 'pet_friendly', 'label': 'Можно с животными', 'icon': 'bi-paw'},
                ],
            },
        ]

        # Filter the full list by the selected amenities.  Only include
        # categories that have at least one selected option.
        filtered_categories: list[dict] = []
        for cat in amenities_categories_full:
    ...
    for r in photo_rows:
        ...
        # preferred; fallback to index access.
        f_name = None
        img_data = None
        try:
        ...
        if img_data is not None and not isinstance(img_data, str):
            try:
                ...
                img_data = str(img_data)
        # Build the photo source string.  Prefer base64-encoded data (either from
        # the database or by reading the file from disk).  This ensures that
        # photos display even when the uploaded_room_image route is not
        # accessible.
        src = None
        if img_data:
            # Use image_data from database as is, prefixing with MIME
            mime = 'image/jpeg'
            try:
                fn_temp = f_name or ''
...
def room_preview():
    ...
    try:
        ...
        # when they are not yet persisted to the permanent room_photos table.
        ...
        # temporary photo table.  Only the file names are fetched to avoid
        # loading large base64 strings into memory.  When filenames are found,
        # their images are read from disk and encoded in the same way as above.
        if not photos:
            sid_local = session.get('_upload_temp_id')
            if sid_local:
                try:
                    ...
                    for row in rows:
                        ...
                        if fname_key:
                            try:
                                # Attempt to read the file from disk and base64 encode
                                file_path2 = os.path.join(UPLOAD_ROOMS_FOLDER, fname_key)
                                try:
                                    with open(file_path2, 'rb') as f_img2:
                                        img_bytes2 = f_img2.read()
                                    if img_bytes2:
                                        mime2 = 'image/jpeg'
                                        try:
